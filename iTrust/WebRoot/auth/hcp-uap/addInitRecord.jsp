...
if (formIsFilled != null) { // add the record only if the form is filled
	...
	String yearConceived = request.getParameter("yearConceived");
	String weeksInPreg = request.getParameter("weeksPreg");
	String daysInPreg = request.getParameter("daysPreg");
	String laborHours = request.getParameter("laborHours");
	String deliveryType = request.getParameter("deliveryType");
	
	String newRecord_PriorPregs = "";
	
	boolean addPriorPreg = false;
	if(request.getParameter("addPriorPreg") != null && request.getParameter("addPriorPreg").equals("on")){
		addPriorPreg = true;
	}
	...
	if (formError == "" && addPriorPreg) {
		int yearConceivedInt = Integer.parseInt(yearConceived);
		if(yearConceivedInt < 1000 || yearConceivedInt > 9999)
			formError = "Invalid year of conception. Please enter a four-digit number.";
			
		int weeksInPregInt = Integer.parseInt(weeksInPreg);
		int daysInPregInt = Integer.parseInt(daysInPreg);
		
		if(weeksInPregInt < 0 || weeksInPregInt > 42)
			formError = "Invalid number of weeks pregnant. Must be an integer value between 0 and 42.";
		
		if(daysInPregInt < 0 || daysInPregInt > 6)
			formError = "Invalid number of days pregnant. Must be an integer value between 0 and 6.";
	
		if(laborHours != "") {
			double laborHoursDouble = Double.parseDouble(laborHours);
			if(laborHoursDouble < 0.0)
				formError = "Invalid number of hours in labor. Must be a positive decimal value";
		}
	}
	...
	if (formError == "") { //we have no errors so far, so we keep going
		...
		if(addPriorPreg) {
			System.out.println(newRecord_PriorPregs);
			newRecord_PriorPregs = yearConceived + ":" + weeksInPreg + ":" + daysInPreg + ":" + laborHours + ":" + deliveryType;
...
<table class="fTable" align="center">
	...
	<tr>
		<td>
			<input type="checkbox" id="addPriorPreg" name="addPriorPreg"><i>Add a Prior Pregnancy</i>
		</td>
	</tr>
	<tr>
		<td class="subHeaderVertical">Year of Conception:</td>
		<td><input name="yearConceived" value=""/>
	</tr>
	<tr>
		<td class="subHeaderVertical">Weeks Pregnant:</td>
		<td><input name="weeksPreg" value=""/>
	</tr>
	<tr>
		<td class="subHeaderVertical">Days Pregnant:</td>
		<td><input name="daysPreg" value=""/>
	</tr>
	<tr>
		<td class="subHeaderVertical">Hours in Labor:</td>
		<td><input name="laborHours" value=""/>
	</tr>
	<tr>
		<td class="subHeaderVertical">Type of Delivery:</td>
