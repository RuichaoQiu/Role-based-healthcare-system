...
public class SendRemindersActionTest extends TestCase {
	...
	public void testNormalReturn() throws Exception {
		...
		action.sendVisitReminders(SendVisitRemindersAction.ReminderType.IMMUNIZATION_NEEDERS);
		action.sendVisitReminders(SendVisitRemindersAction.ReminderType.IMMUNIZATION_NEEDERS,30);
	}	
	...
	/**
	 * 
	 * testImmunizationMessage
	 */
	public void testmakeImmunizationMessage() throws Exception{
		gen.standardData();
		action = new SendVisitRemindersAction(factory,9000000000L);
		try{
			GetVisitRemindersAction getReminders = new GetVisitRemindersAction(this.factory,9000000003L);
			VisitReminderReturnForm reminder2 =  getReminders.getVisitReminders(GetVisitRemindersAction.ReminderType.IMMUNIZATION_NEEDERS).get(1);  
			MessageBean reminder = action.makeImmunizationMessage(reminder2);
			assertEquals(reminder.getFrom(),9000000009L);
			assertEquals("Reminder: You are missing neccessary immunizations",reminder.getSubject());
