...
public class TransactionDAO {
	...
	/**
	 * Fetch that finds all transactions of a certain type that occurred between the start and the end Timestamps
	 * @param start - Earliest wanted Timestamp
	 * @param end - Latest wanted Timestamp
	 * @param type - Specific TransactionType wanted
	 * @return List of Transaction Beans that represents the results of the parameterized query
	 * @throws DBException
	 */
	public List<TransactionBean> getTransactionsBetweenOfType (Timestamp start, Timestamp end, TransactionType type) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		
		try {
			conn = factory.getConnection();
			if (type.equals(TransactionType.GENERIC_CODE)){
				ps = conn
						.prepareStatement("SELECT * FROM transactionlog WHERE timeLogged >= ? AND timeLogged <= ? ORDER BY timeLogged DESC");
			...
			{
				ps = conn
						.prepareStatement("SELECT * FROM transactionlog WHERE transactionCode = ? " +
