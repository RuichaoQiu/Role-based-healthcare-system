...
public class DiagnosesDAO {
	...
	/**
	 * Gets a local zip code count, regional count, state count, and all counts of a specified diagnosis code
	 * by Suchat Jiranuntarat
	 * @param icdCode The diagnosis code
	 * @param zipCode The zip code to evaluate
	 * @param lower The starting date
	 * @param upper The ending date
	 ...
	 * @throws DBException
	 */
	public DiagnosisStatisticsBean getDiagnosisCountsRSN(String icdCode, String zipCode, java.util.Date lower, java.util.Date upper) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		DiagnosisStatisticsBean dsBean = null;
		try {
			conn = factory.getConnection();
			ps = countDiagnosisSQLHelper(icdCode, zipCode, lower, upper, conn, zipCode.length());
			ResultSet rs = ps.executeQuery();
			rs.last();
			int local = rs.getRow();
			
			ps = countDiagnosisSQLHelper(icdCode, zipCode, lower, upper, conn, 3);
			rs = ps.executeQuery();
			rs.last();
			int region = rs.getRow();
			
			ps = countDiagnosisSQLHelper(icdCode, zipCode, lower, upper, conn, 2);
			rs = ps.executeQuery();
			rs.last();
			int state = rs.getRow();
			
			ps = countDiagnosisSQLHelper(icdCode, zipCode, lower, upper, conn, 0);
			rs = ps.executeQuery();
			rs.last();
			int nation = rs.getRow();
			
			dsBean = new DiagnosisStatisticsBean(zipCode, local, region, state, nation, lower, upper);
			rs.close();
			ps.close();
			return dsBean;
		} catch (SQLException e) {
	...
	/**
	 * This is a helper function to help grab the correct query from the SQL database for getDiagnosesCountsRSN
	 * by Suchat Jiranuntarat
	 * @param icdCode The diagnosis code
	 * @param zipCode The zip code to evaluate
	 * @param lower The starting date
	 ...
	 * @param zipCount The number of numbers from zip code being compared
	 * @return a prepared statement for the function to use
	 * @throws DBException
	 */
	private PreparedStatement countDiagnosisSQLHelper(String icdCode,
			String zipCode, java.util.Date lower, java.util.Date upper,
			Connection conn, int zipCount) throws SQLException {
		PreparedStatement ps;
		ps = conn.prepareStatement("SELECT * FROM ovdiagnosis INNER JOIN officevisits ON ovdiagnosis.VisitID=officevisits.ID INNER JOIN patients ON officevisits.PatientID=patients.MID WHERE ICDCode=? AND zip LIKE ? AND visitDate >= ? AND visitDate <= ? ");
		ps.setString(1, icdCode);
		ps.setString(2, zipCode.substring(0, zipCount) + "%");
		ps.setTimestamp(3, new Timestamp(lower.getTime()));
		// add 1 day's worth to include the upper
		ps.setTimestamp(4, new Timestamp(upper.getTime() + 1000L * 60L * 60 * 24L));
		return ps;
	...
	/**
	 * Gets a weekly local zip code count, regional count, state count, and all counts of a specified diagnosis code over a time period
	 * by Suchat Jiranuntarat
	 * @param icdCode The diagnosis code
	 * @param zipCode The zip code to evaluate
	 * @param lower The starting date
	 ...
	 * @throws DBException
	 */
	public ArrayList<DiagnosisStatisticsBean> getWeeklyCountsRSN(String icdCode, String zipCode, java.util.Date lower, java.util.Date upper) throws DBException {
		Calendar cal = Calendar.getInstance();
		cal.setTime(lower);
		Date lowerDate = cal.getTime();
		cal.add(Calendar.HOUR, 24*6);
		...
		do {
			DiagnosisStatisticsBean db = getDiagnosisCountsRSN(icdCode, zipCode, lowerDate, upperDate);
			weekStats.add(db);
